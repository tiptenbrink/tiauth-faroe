...
from tiauth_faroe.client.logic import ActionErrorResult, ActionResult, GetSessionActionSuccessResult, JSONValue, get_session, JSONDict, parse_action_invocation_response
...
class AsyncClient:
    ...
    async def get_session(self, session_token: str) -> GetSessionActionSuccessResult | ActionErrorResult:
        return await send_gen_async(get_session(session_token), 'get_session', self.manage_action_invocation_request)


T = TypeVar('T')


async def send_gen_async(gen: Generator[JSONDict, ActionResult, T], action: str, send_request: Callable[[str, JSONDict], Awaitable[ActionResult]]) -> T:
    args = next(gen)
    try:
...
class SyncClient:
    ...
    def get_session(self, session_token: str) -> GetSessionActionSuccessResult | ActionErrorResult:
        return send_gen_sync(get_session(session_token), 'get_session', self.manage_action_invocation_request)



def send_gen_sync(gen: Generator[JSONDict, ActionResult, T], action: str, send_request: Callable[[str, JSONDict], ActionResult]) -> T:
    args = next(gen)
    try:
